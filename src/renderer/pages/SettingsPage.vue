      ...
      <NFlex vertical :size="24">
        ...
        <NCard v-show="activeSettingKey === 'close-behavior'" title="关闭行为详细设置">
          <NFlex vertical :size="16">
            ...
            <NFormItem label="提示设置">
              <NCheckbox 
                v-model:checked="settings.dontShowCloseDialog" 
            ...
            <NAlert v-if="settings.dontShowCloseDialog" type="success" show-icon>
              <template #header>设置已生效</template>
              您的关闭行为偏好已保存。现在点击关闭按钮将直接执行"{{ settings.closeAction === 'quit' ? '退出应用' : '最小化到托盘' }}"操作。
              <br>如需修改，可在此页面随时调整设置。
            </NAlert>
          </NFlex>
        </NCard>
        ...
        <NCard v-show="activeSettingKey === 'startup-behavior'" title="启动行为详细设置">
          <NFlex vertical :size="16">
            ...
            <NFormItem label="自启动设置">
              <NCheckbox 
                v-model:checked="settings.autoLaunch" 
                @update:checked="updateSetting"
              >
                <NFlex align="center" :size="8">
                  <div>
...
const currentSectionInfo = computed(() => {
  const sections = {
    'close-behavior': {
      title: '关闭行为设置',
      icon: Power,
      description: '配置点击关闭按钮时的行为，可选择退出应用或最小化到托盘。启用"记住我的选择"后，设置将永久保存。'
    },
    'startup-behavior': {
      title: '启动行为设置',
      icon: Rocket,
      description: '配置应用启动时的行为，包括是否最小化启动和开机自启动等选项。'
    },
    'appearance': {
      title: '外观设置',
      icon: Sun,
...
const updateSetting = async () => {
  ...
  try {
    ...
    const settingsData = JSON.parse(JSON.stringify({
      dontShowCloseDialog: settings.dontShowCloseDialog,
